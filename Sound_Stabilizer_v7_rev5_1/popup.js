// popup.js v33 (Финальный рабочий код с исправленными ошибками tabs.id)
...
// --- КОНСТАНТЫ И ПЕРЕМЕННЫЕ ДЛЯ ГРАФИКА И ОТЛАДКИ ---
...
let dragged = false; // Флаг для отличия клика от перетаскивания
let dragStartX = 0;
...
function drawDebugChart() {
    ...
    // Очистка холста и отрисовка сетки/линий
    ...
    dbLevels.forEach(db => {
        let rawValue = (Math.pow(10, db / 20) * 255) || 0;
        let y = chartHeight * (1 - rawValue / 255);
        debugCtx.beginPath();
        debugCtx.moveTo(0, y);
        debugCtx.lineTo(debugChartCanvas.width, y);
        debugCtx.stroke();
    });

    debugCtx.strokeStyle = '#eee';
    const numVerticalLines = 5;
    for (let i = 0; i <= numVerticalLines; i++) {
        const x = (chartWidth / numVerticalLines) * i;
        debugCtx.beginPath();
        debugCtx.moveTo(x, 0);
        debugCtx.lineTo(x, chartHeight);
        debugCtx.stroke();
    }
    ...
    if (visibleData.length >= 2) {
        ...
        visibleData.forEach((point, index) => {
            if (index === 0) return;
            const x1 = widthStep * (index - 1);
            const x2 = widthStep * index;
            const y1_pre = chartHeight * (1 - visibleData[index - 1].pre / 255);
            const y2_pre = chartHeight * (1 - point.pre / 255);
            const y1_post = chartHeight * (1 - visibleData[index - 1].post / 255);
            const y2_post = chartHeight * (1 - point.post / 255);
    ...
    // Отрисовка черной полупрозрачной иконки паузы в левом верхнем углу
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (tabs && tabs.length > 0) {
        ...
        if (activeTab.url.startsWith('chrome://') || activeTab.url.startsWith('chrome-extension://')) {
            document.getElementById('statusArea').textContent = 'Статус: Недоступно';
            document.getElementById('statusArea').style.color = '#555';
            ...
            try {
                ...
                console.error("Критическая ошибка инжекции injected_script.js или загрузки настроек:", error);
