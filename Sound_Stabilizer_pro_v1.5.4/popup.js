// popup.js v2025 ULTIMATE FULL (All Features Restored)
(function() {
    "use strict";
    ...
    function draw() {
        ...
        // 3. Отрисовка линий (Красная - до, Зеленая - после)
        ...
        // 4. Вертикальный курсор "Сейчас" (Изящный вариант)
        if (!isPaused) {
    ...
    function updateBar(v) {
        ...
        let db = v === 0 ? -60 : Math.max(-60, Math.round(20 * Math.log10(v / 255)));
    ...
    // --- ПОДСКАЗКИ (ПОЛНАЯ ВЕРСИЯ С ПРОВЕРКОЙ ГРАНИЦ) ---
    function setupTooltips() {
        document.querySelectorAll('.tooltip-trigger').forEach(trig => {
            trig.onmouseenter = function() {
                ...
                if (!tip) return;
                tip.style.visibility = 'visible'; tip.style.opacity = '1'; tip.style.transform = 'translateY(0)';
                const r = this.getBoundingClientRect(), pR = document.body.getBoundingClientRect();
                let top = r.top, left = r.right + 10;
                if (left + tip.offsetWidth > pR.right - 5) left = r.left - tip.offsetWidth - 10;
                if (left < pR.left + 5) left = pR.left + 5;
                if (top + tip.offsetHeight > pR.bottom - 5) top = r.top - tip.offsetHeight - 10;
                if (top < pR.top + 5) top = pR.top + 5;
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        el.btn.onclick = async () => {
            const originalText = el.btn.textContent;
            el.btn.textContent = "Ждем..."; // Визуальный отклик
            ...
            if (r && r.status) {
                ...
                // ПЛАН Б: Если ответа нет (специфика CVH), запрашиваем статус принудительно
                setTimeout(async () => {
                    const check = await sendCmd("getStatus");
                    if (check && check.status) updateUI(check.status);
                    else el.btn.textContent = originalText; // Вернуть исходный текст, если ошибка
                }, 600);
            }
        };
        ...
        // --- ИНТЕРАКТИВ ГРАФИКА (MOVE / ZOOM / PAUSE) ---
